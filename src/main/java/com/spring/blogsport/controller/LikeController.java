...
public class LikeController {
    ...
    // @ResponseBody
    // public Map<String, Object> addAndRemoveLike(@PathVariable Long postId, @AuthenticationPrincipal UserDetails userDetails) {
    //     User user = userService.findByEmail(userDetails.getUsername()).orElseThrow();
    //     likeService.addAndRemoveLike(postId, user.getId());
    //     long updatedLikeCount = likeService.countLikes(postId); // Atualiza o número de likes
    //     Map<String, Object> response = new HashMap<>();
    //     response.put("likeCount", updatedLikeCount);
    //     return response;
    // }

    // // Esse método recebe a requisição do postList.html e rediciona para a página inicial
